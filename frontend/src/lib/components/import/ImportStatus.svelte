...
{#if currentStatus}
  <div class="p-4 rounded-lg border {statusBg(currentStatus.status)}" data-testid="import-status-current">
...
{#if imports.length > 0}
  <div class="mt-4 space-y-2">
    ...
    {/if}
    {#each imports as imp, i}
      <div class="flex items-center justify-between p-3 rounded-lg bg-surface-800/50 border border-surface-700" data-testid="import-history-{i}">
        <div class="flex items-center gap-3">
          ...
          {#if imp.status !== 'processing' && imp.status !== 'cancelling'}
            ...
            {:else}
              <button
                ...
                data-testid="import-delete-btn-{imp.batch_id}"
              >
